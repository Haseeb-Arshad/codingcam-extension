...
export class CodingCamDependencies {
  ...
  private githubDownloadUrl = 'https://github.com/Haseeb-Arshad/codingcam-cli/releases/latest/download'; // Hypothetical repo
  private githubReleasesUrl = 'https://api.github.com/repos/Haseeb-Arshad/codingcam-cli/releases/latest'; // Hypothetical repo
  ...
  public getCliLocationGlobal(): string | undefined {
    ...
    const binaryName = `codingcam-cli${Desktop.isWindows() ? '.exe' : ''}`;
          ...
          this.options.getSetting(
            ...
            (accessed: Setting) => {
              ...
              if (lastAccessed && lastAccessed + fourHours > now) {
                this.logger.debug(
                  `Skip checking for codingcam-cli updates because recently checked ${
                    now - lastAccessed
                  } seconds ago.`,
                );
                callback(true);
