...
export class Options {
  ...
  constructor(logger: Logger, resourcesFolder: string) {
    this.logger = logger;
    this.configFile = path.join(Desktop.getHomeDirectory(), '.codingcam.cfg');
    ...
    try {
      ...
      if (`${err}`.includes('spawn EPERM')) {
        vscode.window.showErrorMessage(
          'Microsoft Defender is blocking CodingCam. Please allow CodingCam to run so it can upload code stats to your dashboard.',
        );
      }
      return '';
    }
  ...
  public async getApiUrl(checkSettingsFile = false): Promise<string> {
    ...
    if (!apiUrl) apiUrl = 'https://api.codingcam.com/api/v1';
